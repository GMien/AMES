...
class UploadHandler {
    ...
    public function handleUpload($uploadDirectory, $name = null){
            ...
            $this->toBytes(ini_get('upload_max_filesize')) < $this->sizeLimit)
		{
            ...
            return array('error'=>"Server error. Increase post_max_size and upload_max_filesize to ".$this->sizeLimit.
			','.$this->toBytes(ini_get('post_max_size')).','.  $this->toBytes(ini_get('upload_max_filesize')));
        }
        ...
        if ($folderInaccessible){
            return array('error' => "Server error. Uploads directory isn't writable" . ((!$isWin) ? " or executable." : ".") .$uploadDirectory);
        }
    ...
    protected function getUniqueTargetPath($uploadDirectory, $filename)
    {
        ...
        while (file_exists($uploadDirectory . DIRECTORY_SEPARATOR . $unique . $ext)  ||
		   ( trim($this->SecondDirectory)!='' && file_exists($this->SecondDirectory . DIRECTORY_SEPARATOR . $unique . $ext))		
		)
		{
            $suffix += 1;//rand(1, 999);
